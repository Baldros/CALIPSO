...
def salvar_imagens(subpasta: str, nome_imagem: str):
    ...
    try:
        ...
        print('Deu um erro na função salvar_imagens')


def plot_media_desvio_padrao(dado: pd.DataFrame or pd.Series, niveis_altitude: np.ndarray, ax: plt.Axes, cor: str or None = None):
    """
    ...
    #print('Finalizado - plot_media_desvio_padrao\n')
def plot_media_deteccoes_1d(media_sazonal_deteccoes: pd.DataFrame, niveis_altitude: np.ndarray, area: str, salvar=True):
    ...
    for estacao in estacoes:
        try:
            ...
            print(f'Deu problema na estação {estacao}')
    ...
    if salvar == True:
        try:
            ...
            print('Deu merda no gráfico do plot_media_deteccoes_1d')
    #print('Finalizado - plot_media_desvio_padrao\n')

def plot_deteccoes_sazonais_2d(media_sazonal_deteccoes: pd.DataFrame, niveis_altitude: np.ndarray, area: str, informacoes_variavel: dict, salvar=True):
    """
    ...
    try:
        ...
        print('Deu merda no gráfico do plot_deteccoes_sazonais_2d no fig.legend') 
    fig.tight_layout()
    ...
    if salvar == True:
        try:
            ...
            print('Deu merda no gráfico do plot_deteccoes_sazonais_2d')
    #print('Finalizado - plot_deteccoes_sazonais_2d\n')
...
def plot_media_movel_deteccoes_eoa(dados_deteccao: pd.DataFrame, dados_eoa: pd.DataFrame, area, salvar: bool = True):
    ...
    if salvar:
        try:
            ...
            print('Deu merda no gráfico do plot_media_movel_deteccoes_eoa')
    #print('Finalizado - plot_media_movel_deteccoes_eoa\n')
def plot_coeficiente_extincao(media_sazonal: pd.DataFrame, informacoes_variaveis: dict, niveis_altitude: np.ndarray, area: str, salvar=True):
    ...
    if salvar:
        try:
            plt.savefig(f"Imagens/Imagens_L3/media_sazonal_coef_extincao_espessura_{area}.png", bbox_inches='tight', dpi=500)
        except:
            print('Deu merda no gráfico do plot_coeficiente_extincao')

    #print('Finalizado - plot_coeficiente_extincao\n')
def plot_multiplo_perfil_deteccao(perfil_media_sazonal: dict, info_variaveis: dict, niveis_altitude: np.ndarray, salvar: bool = True):
    """
    ...
    if salvar == True:
        try:
            ...
            print('Deu merda no gráfico do plot_multiplo_perfil_deteccao')
    #print('Finalizado - plot_multiplo_perfil_deteccao\n')
    
def plots_multiplos_perfil_ce_eoa(perfil_media_sazonal: dict, info_variaveis: dict, niveis_altitude: np.ndarray, salvar: bool = True):
    """
    ...
    for i in [0,3,6,9]:
        ...
        for estacao, axs_interno in zip(estacoes, axs):
            try:
                ...
                for (area, media_sazonal), ax in zip(list(perfil_media_sazonal.items())[i:i+3], axs_interno):
                    ...
                    # Configurações do gráfico
                    ax.set_xlim(0,0.5) # Trecho Alterado:
                    ax.set_xticks(np.arange(0,0.4,.1)) # Trecho alterado
                    ax.set_ylim(-0.5, 8)
                ...
                print(f'Problema na estação {estacao}')
        ...
        if salvar:
            try:
                ...
                print('Deu merda no gráfico do plots_multiplos_perfil_ce_eoa')
        #print('Finalizado - plots_multiplos_perfil_ce_eoa\n')
        
#Comentário divisório:            
'''
    Acredito eu que seja a partir desse momento que o código de plotagem comece a
ficar diferente, por conta disso, vou colocar esse comentário ancora aqui, para separar
as duas partes, ficando mais fácil visualizar.
'''

def plot_multiplo_media_movel_tendencia(dados: dict, variavel: str, info_variaveis: dict, salvar: bool = True):
    """
    ...
    if salvar == True:
        try:
            ...
            print('Deu merda no gráfico do plot_multiplo_media_movel_tendencia')

    #print('Finalizado - plot_multiplo_media_movel_tendencia\n')
def plot_media_movel_deteccoes_2d(dados_deteccao: pd.DataFrame, info_variavel: dict, area: str, salvar: bool = True):
    """
    ...
    # ax.set_yticks(np.arange(0, 13, 2))
    ax.grid(True)
    legenda = [Line2D([0], [0], color=info_variavel['Tipos_Aerossóis'][aerossol], lw=2) for aerossol in dados_deteccao.columns.unique(0)]
    lgd = fig.legend(legenda,dados_deteccao.columns.unique(0), loc='upper center', bbox_to_anchor=(0.5, 0), ncol=3, borderaxespad=0.)
    #try:
    #    lgd = fig.legend(media_movel.columns.unique(0), loc='upper center', 
    #            bbox_to_anchor=(0.5, 0), ncol=3, borderaxespad=0.)
    #except:
    #    print('Deu merda no gráfico do plot_media_movel_deteccoes_2d no lgd')
    ...
    if salvar:
        try:
            ...
            print('Deu merda no gráfico do plot_media_movel_deteccoes_2d')
    #print('Finalizado - plot_media_movel_deteccoes_2d\n')
#print('Plotagens Finalizadas')