...
def reordenando_array(dado: np.ndarray, latitudes: np.ndarray) -> list:
    """
...
def selecao_area(dado: SD, coordenadas: dict, dataset: str) -> np.ndarray:
    """
...
def convert_dicionario_2d_dataframe(dicionario_entrada: dict, info_variavel: dict) -> dict:
    """
...
def calc_media_sazonal_anual_1d(dicionario_entrada: dict) -> pd.DataFrame:
    """
...
def calc_media_sazonal_anual_2d(dicionario_entrada: dict) -> pd.DataFrame:
    """
    Retorna dataframe com média para cada estação de cada ano
    """
...
def calc_media_sazonal_1d(dataframe: pd.DataFrame) -> pd.DataFrame or pd.Series:
    """
...
def calc_media_sazonal_2d(dataframe: pd.DataFrame) -> pd.DataFrame or pd.Series:
    """
    Retorna dataframe com a média sazonal a partir de um dataframe que tenha os valores para estação de cada ano
    """
...
def calc_soma_deteccoes_coluna(dado: dict) -> dict:
    """
...
def string2json(string, nome):
    '''
        Função construida para facilitar
    a conversão de uma string em json.
    '''
    # Exporta para um arquivo
    with open(f'{nome}.json', 'w') as arq:
        json.dump(string, arq)
        
    print('Json criado com sucesso :>')

def json_reader(diretorio_json):
    '''
        Função construida para ler o json,
    de modo a ajudar a deixar o código mais limpo.
    '''

    with open(diretorio_json,'r') as arq:
        info = arq.read()
